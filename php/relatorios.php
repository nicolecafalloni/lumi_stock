...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt) {
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            if ((int) $row['quantidade'] < 10)
                $totais['baixoEstoque']++;

            // Categoria sums
            $cat = $row['categoria'] ?: 'Outros';
            if (!isset($categoriaSums[$cat]))
                $categoriaSums[$cat] = 0.0;
                    ...
                    <div class="rel-row">
                        <div class="rel-field">
                            ...
                            <select id="tipoRelatorio" name="tipoRelatorio" required>
                                <option value="produtos">Produtos
                                <option>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if ((int) $p['quantidade'] === 0)
                                            echo "<span class='rel-status rel-sem'>Sem estoque</span>";
                                        elseif ((int) $p['quantidade'] < 10)
                                            echo "<span class='rel-status rel-baixo'>Baixo</span>";
                                        else
                                            echo "<span class='rel-status rel-normal'>Normal</span>";
                                        ?>
                                    </td>
